...
class Config:
    def __init__(
        ...
        assert isinstance(auto_scale_fft, bool)
        self.auto_scale_fft = auto_scale_fft
        if auto_scale_fft:
            debug(f"Auto FFT scaling enabled: base size {fft_size} will scale with sample rate")
        else:
            debug(f"Auto FFT scaling disabled: using fixed size {fft_size}")
